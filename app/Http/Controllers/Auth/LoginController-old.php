...
class LoginController extends Controller
{
    ...
    */
    ...
    /*public function login(Request $request)   // Very old login functionality
    {
    ...
    public function login(Request $request)
    {
        ...
        if ( ($auth !== NULL) && (!in_array('LDAP', $auth->getRoleNames()->toArray())) ) {
            ...
            try {
                ...
                $userAdd = User::updateOrCreate(
                    ...
                    [
                        ...
                        'role' => 'LDAP', // Assign 'LDAP' as the default role in your database
                        'password' => bcrypt($credentials['password']), // Store bcrypt password
                    ]
                );        
                // Assign the 'User' role
                if ($userAdd->exists) {
                    $role = Role::where('name', 'LDAP')->first();
