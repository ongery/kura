<template>
    <div>
                            ...
                            <div class="card-body py-3">
                                <div class="table-responsive">
                                    <table class="table table-row-dashed table-row-gray-300 align-middle p-0">
                                        ...
                                        <tbody>
                                            <tr v-for="mode in crAlldata" :key="mode.id">
                                                <td>
                                                    <a style="color: #258c37;" href="javascript:void(0)" @click.prevent="editModal(mode)">
                                                        {{ mode.DocNum }}
                                                    </a>
                                                </td>
                                                <td>{{ mode.Name }}</td>
                                                ...
                                                <td>
                                                    <a href="javascript:void(0)" @click.prevent="deleteCorporateRisk(mode.id)">
        ...
        <!-- Modal -->
        <div class="modal fade" id="addNew" tabindex="-1" role="dialog" aria-labelledby="addNewLabel" aria-hidden="true" data-backdrop="static" data-keyboard="false">
            <div class="modal-dialog" role="document">
                <div class="modal-content">
                    <div class="modal-header">
                        <h5 class="modal-title" id="addNewLabel">Corporate Risks</h5>
                        <button type="button" class="close" data-dismiss="modal" aria-label="Close" @click="closeModel">
                            <span aria-hidden="true">&times;</span>
                        </button>
                    </div>
                    <form @submit.prevent="editmode ? updateCorporateRisk() : createCorporateRisk()">
                        <input v-if="editmode" type="hidden" name="corporateRiskId" id="corporateRiskId" v-model="form.id" />
...
export default {
    ...
    methods: {
        ...
        createCorporateRisk() {
            const token = this.getAuthToken();
            this.$Progress.start()
            this.form.post('/api/corporate-risks',{
                withCredentials: true,
                headers: {
                    'Authorization': `Bearer ${token}`
                }
        ...
        deleteCorporateRisk(id) {
            swal.fire({
                ...
                if (result.value) {
                    this.form.delete('/api/corporate-risks/' + id, {
                        withCredentials: true,
                        headers: {
                            'Authorization': `Bearer ${token}`
                        }
        ...
        updateCorporateRisk() {
            const token = this.getAuthToken();
            this.$Progress.start()
            this.form.put('/api/corporate-risks/'+this.form.id, {
                withCredentials: true,
                headers: {
                    'Authorization': `Bearer ${token}`
                }
